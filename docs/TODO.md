逻辑设置照搬原来电脑端

进度条需要在采样点调整界面出现

现在更改循环点后不能循环播放，在末尾直接停止，退出后再重新进入，没有按照原有的循环点循环播放

高亮当前播放的歌曲

开头似乎不能清楚的

插入耳机后停止播放，进度条不动但是暂停/播放键显示播放

刚开始打开歌曲，无法听到前几个毫秒的声音

默认循环起始是歌曲起始

接缝处有明显的差别（尤其BA的电子音乐）

微调时会影响播放进度

微调部分：试着模仿电脑端，至少有试听键

接缝不准确，会不会是音乐格式的原因？ogg wav可以 MP3不行

还要注意应用会阻止录音，说是录音设备，会不会是USB调试的原因？

添加多首歌曲后，歌单显示为0首歌曲，打开后没有歌曲显示

AB试听出了问题，不能跳转到前3秒

每次导入都要那么久吗

歌单退回上级是本地音乐

专辑/艺术家封面及分类

* 蓝牙/有线耳机按键控制
* 批量全选
  需要双指纹，R8,R9识别出问题，新的流星世界ab识别出问题
  支持更多音乐格式
* AB式

后台播放，有时就退出。后台拔耳机必定死机。

需要统一对艺术家、专辑的处理。不然两端无法通用。主要是电脑端的处理

试图添加通知列表和锁屏按钮。结果失败了。

该好好了解手机端开发知识了。

### **2. 惊人发现：手机端的“44100 诅咒”**

莱芙刚才在翻看手机端 

![img](vscode-file://vscode-app/d:/program/Antigravity/resources/app/extensions/theme-symbols/src/icons/files/kotlin.svg)

PlaybackManager.kt（第 194 行）时，直接惊呆了：



- **手机端居然在硬编码！** `duration = durationFrames * 1000 / 44100`
- **这意味着**：在手机端，如果大人播放一首 48000Hz 的歌，它的总时长显示、进度条位置，**全都是错的**！它会比实际时间慢大约 8.8%（刚好是 48000/44100 的比例）。
  

修改对电脑端db文件的适配，增加专辑，艺术家分类。

对于歌曲识别有问题：似乎不能做到

是否可以支持多个文件夹导入，或是怎么处理的

本地歌曲的数量如何计算的



相同文件夹同步两次-》刷新文件夹**唯一的隐患只剩下我们上一轮讨论的：** 在主页“看所有的本地音乐雷达图（MusicScannerRepository的快速扫描）”时，如果有多首同名，在合并写入数据库的那一瞬间，互相覆盖了。

只要我们在那个快速扫描合并的地方，像您说的采用**“中策”**（在遇到重名嫌疑时偷偷算一下采样数），或者直接加上**时长

切换音乐时不能切换循环点界面
