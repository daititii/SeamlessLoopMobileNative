...
- [x] 蓝牙/有线耳机按键控制 - MediaControlManager已实现媒体按钮，但耳机物理按键支持需要进一步测试
...
- [x] 又开始不能列表播放与随机播放（只能单曲循环），即使有歌曲循环点在末尾，歌曲播放到此处，就会快速向下滑动越过大量歌曲，然后停在某首歌曲 - PlayMode枚举已定义LIST_LOOP/SINGLE_LOOP/SHUFFLE，但播放队列逻辑可能有bug
- [ ] 手机端也要支持排行榜界面 - 完全未实现
...
- [x] 拖动进度条应当继续之前的播放 - 可能存在触摸事件处理问题
- [ ] 增加手动同步文件夹功能，或者说当前是怎么同步的 - 当前仅支持自动扫描，无手动触发点
- [ ] 支持显示别名
- [ ] 在右下角增加一个播放列表
- [ ] 在一个音乐专辑退出后就返回了专辑界面的最上层
- [ ] 歌曲模式必须在切换歌曲后才可以具体实现，由此影响到切歌机制。
- [ ] 初步加载时，似乎不能正确加载上次的播放模式
- [ ] 尝试增加以下功能
