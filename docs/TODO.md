...
- [X] 初步加载时，似乎不能正确加载上次的播放模式。实际上正确加载了，但是界面不能正确显示。。。
- [ ] 还是要加一个歌曲详情页面，或者主界面加一个专辑名字也凑合
- [ ] 当前随机播放没有问题吧？感觉一首歌有放了好多次
- [ ] 要不要检测是否内存泄漏？比如isLeak
- [ ] 
- [ ] 尝试增加以下功能
...
- [ ] 开头似乎不能清楚的
- [ ] 刚开始打开歌曲，无法听到前几个毫秒的声音
- [ ] 支持更多音乐格式 - AudioDecoder通过Android
  MediaExtractor支持MP3/WAV/OGG/AAC等格式，但某些边缘格式可能不支持
- [ ] 手机端也要支持排行榜界面 - 完全未实现
- [ ] 可以考虑增加图标了 - 未添加新图标资源
- [ ] 支持我的收藏功能，默认收藏的歌单可以更改 - 未实现相关逻辑
- [ ] 增加手动同步文件夹功能，或者说当前是怎么同步的 - 当前仅支持自动扫描，无手动触发点
- [ ] 支持显示,修改别名
- [ ] 在右下角增加一个播放列表
- [ ] 在一个音乐专辑退出后就返回了专辑界面的最上层
- [ ] 歌曲模式必须在切换歌曲后才可以具体实现，由此影响到切歌机制。
- [ ] 初步加载时，似乎不能正确加载上次的播放模式。实际上正确加载了，但是界面不能正确显示。。。
- [ ] 还是要加一个歌曲详情页面，或者主界面加一个专辑名字也凑合
- [ ] 当前随机播放没有问题吧？感觉一首歌有放了好多次
- [ ] 要不要检测是否内存泄漏？比如isLeak
...
第一次播放的循环歌曲，不能循环，需要应用并试听才能循环，不然播放到结尾停止
...
将我的歌单从主界面分离，变成单个模块，界面类似专辑

把无法后台清除的设置删掉

写一份计划书方便AI生成？或者整理一下当前目标——其实UI设计很大程度可以作为HCL作业

标记正在播放的歌曲所在文件夹

我们这个是怎么做到的？循环播放底层播放引擎与cotlin应用层是什么关系？

通知界面操控应当和网易云一样

按下应用并试听开启了强制单曲循环，此时仍然显示非单曲循环，不是不好，只是有点奇怪。至少显示播放模式的方式要修改一下：什么操控？什么显示？或者搞一个类似播放状态的状态机也可以-----这个问题的启示：播放引擎与ui功能的解耦不完全，可以在其它代码开头看是否有相关的不合适的底层引用

不过试图修改这个问题，似乎导致了试听时变成了顺序播放，尽管此时显示单曲循环。AB时的问题

又经过一次修改，目前变成了AB式播放下，可以先到A段可以正常显示两首歌曲的合体，随后到末尾单曲循环时，则切换到B段播放，不能回到A段（似乎又没有了？难道是leak导致）

尽早做成live 修改模式

似乎详情界面，在非按键的地方的空白处按下，会导致重头播放，怎么回事（似乎又没有了？难道是leak导致）

要不把无缝单曲循环也做成可选？

目前我们的数据同步：手机没有的，电脑有，那么就去掉不储存

一种随机模式：始终不会重复播放歌曲！！！根据听歌次数判定，越多越往前

如果非单曲循环下，可循环歌曲，末尾再加两秒淡出，不然停止太突兀？怎么处理要研究

似乎将AB段的a单独拿到歌单就无法循环播放，只能顺序

如果匹配时还是同名，调用库还是怎么做呢？

AB模式下，按下确认应用并试听后，发生怪事：正常播放完AB合体内容后，歌曲竟然切换到B部分循环播放，此后在下方的控制区非按键的地方的空白处按下会跳转到某一首歌从头播放，
